...
import { useUpdateTraveler } from "@/hooks/user.hook";
...
const ProfilePage = () => {
  ...
  const { data: traveler, isPending: isLoadingTraveler } = useGetMe();
  const {
    _id: travelerId,
    name,
    phone,
  ...
  const handleUpdateProfile = async (values: TUser) => {
    ...
    updateTraveler({ _id: travelerId, formData });
  };
