'use client'
import { useCreateComment, useDeleteComment, useGetAllComment, useUpdateComment } from '@/hooks/comment.hook';
import { TPost } from '@/types/post';
import { Button, List, Form, Pagination, Skeleton, Avatar, Modal, Dropdown, Menu, message } from 'antd';
...
const Comments = ({ post }: { post: TPost }) => {
    ...
    const [editingComment, setEditingComment] = useState<Partial<TComment> | null>(null);
    ...
    const handleUpdateComment = async (values: TComment) => {
        ...
        if (editingComment?._id) {
            updateCommentMutate({ _id: editingComment?._id, comment: values?.comment }, {
                onSuccess: () => {
    ...
    <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-2 mb-4">
      ...
      <Form className={`${!user?.data && 'blur pointer-events-none'}`} form={form} onFinish={handleCreateComment} layout='vertical'>
                        ...
                        renderItem={(comment: TComment) => (
                            <List.Item key={comment._id} className='bg-white rounded-md shadow my-2 hover:shadow-lg transition-shadow duration-200'>
                                <div className='flex items-start gap-3 p-4 w-full'>
                                    <Avatar
                                        size="large"
                                        src={comment.user?.profileImg || "https://i.pravatar.cc/150?u=a04258a2462d826712d"}
                                    ...
                                    <div className='flex-grow'>
                                        <div className='flex justify-between items-center'>
                                            <p className='font-semibold text-gray-800 cursor-pointer' onClick={() => router.push(`/profile/${comment.user?._id}`)}>
                                                {comment.user?.name}
                                            </p>
                                            <div className='flex items-center gap-1'>
                                                {comment.createdAt && (
                                                    <span className='text-gray-500 text-sm'>
                                                        <ClockCircleOutlined /> {moment(new Date(comment.createdAt)).fromNow()}
                                                    </span>
                                                )}
            ...
            <Modal
                title="Edit Comment"
                visible={!!editingComment?._id}
                onCancel={() => {setEditingComment(null); editForm.resetFields()}}
                footer={null}
            >
